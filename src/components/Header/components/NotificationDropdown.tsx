...
// 메시지 상수 분리
const MESSAGES = {
    NOTIFICATION_CENTER: '알림센터',
    MARK_ALL_READ: '모두 읽음 처리',
    NO_NOTIFICATIONS: '14일 이내에 받은 알림이 없습니다',
    TIME_SUFFIX: (title: string) => `${title}${getPostposition(title)} 할 시간이에요.`,
    DATE_GROUPS: {
        TODAY: '오늘',
        YESTERDAY: '어제',
        THIS_WEEK: '이번 주',
                    ...
                    className={`w-full flex px-[20px] py-[16px] justify-between items-start self-stretch border-b border-[#F0F0F0] ${n.read ? "bg-[#FAFAFA]" : ""}`}>
                        <div className="flex items-start gap-6">
                            ...
                            <div className="flex flex-col justify-center items-start gap-[8px]">
                                <span className={`text-center font-pretendard text-[16px] font-semibold leading-normal ${n.read ? "text-[#4D4D4D]" : "text-[#141414]"}`}>{MESSAGES.TIME_SUFFIX(n.title)}</span>
                                {n.memo && (
            ...
            {isOpen && (
                <div className="absolute right-0 top-11 h-[456px] flex w-[464px] flex-col justify-center items-center rounded-[12px] bg-white shadow-[0_0_16px_0_rgba(0,0,0,0.12)]">
                    ...
                    <div className="flex px-[20px] py-[8px] justify-between items-center self-stretch border-b border-[#F0F0F0]">
                        <h3 className="text-[#141414] text-center font-pretendard text-[18px] font-semibold leading-normal">{MESSAGES.NOTIFICATION_CENTER}</h3>
                        <button onClick={onMarkAllRead} className="flex h-[40px] justify-center items-center gap-[8px]">
                            <span className="text-[#5736FF] text-center font-pretendard text-[16px] font-medium leading-normal">{MESSAGES.MARK_ALL_READ}</span>
                        </button>
                    </div>
                    {/* 알림 리스트 */}
                    {notifications.length === 0 ? (
                        <div className="flex h-[400px] p-[100px_0] flex-col items-center gap-[16px] self-stretch">
                            ...
                            <p className="text-[#707070] text-center font-pretendard text-[16px] font-medium leading-[140%]">{MESSAGES.NO_NOTIFICATIONS}</p>
                        </div>
                    ) : (
                        <div className="w-full flex flex-col h-[400px] items-start self-stretch overflow-y-auto">
                            {renderGroup(MESSAGES.DATE_GROUPS.TODAY, grouped["오늘"] || [])}
                            {renderGroup(MESSAGES.DATE_GROUPS.YESTERDAY, grouped["어제"] || [])}
                            {renderGroup(MESSAGES.DATE_GROUPS.THIS_WEEK, grouped["이번 주"] || [])}
                            {renderGroup(MESSAGES.DATE_GROUPS.LAST_WEEK, grouped["지난 주"] || [])}
                        </div>
                    )}
                </div>
            )}
